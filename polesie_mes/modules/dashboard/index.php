...
if ($readyOrdersCount > 0) {
    $docsIssues[] = [
        ...
        'recommendation' => 'Проверить наличие сопроводительной документации',
...
$pendingDocsCount = $readyOrdersCount;

// ==========================================
// ДАННЫЕ ДЛЯ ГРАФИКОВ
// ==========================================

// Динамика заказов по месяцам (6 месяцев)
$stmt = $db->query("
    SELECT 
...
$ordersByStatus = [
    ['status' => 'Новые', 'count' => $stats['orders']['new_orders'] ?? 0, 'color' => '#5ac8fa'],
    ['status' => 'В производстве', 'count' => $stats['orders']['production_orders'] ?? 0, 'color' => '#FF6B6B'],
    ['status' => 'На контроле качества', 'count' => $stats['orders']['qc_orders'] ?? 0, 'color' => '#ffd60a'],
    ['status' => 'Готовы', 'count' => $stats['orders']['ready_orders'] ?? 0, 'color' => '#30d158'],
    ['status' => 'Завершены', 'count' => $stats['orders']['completed_orders'] ?? 0, 'color' => '#9f7aea']
];

// Выполнение производственных заданий по неделям (4 недели)
$stmt = $db->query("
    SELECT 
...
");
$productionByWeek = $stmt->fetchAll();

// Статистика по цехам/участкам (если есть разделение)
$stmt = $db->query("
    SELECT 
        workshop_name,
        COUNT(*) as total_tasks,
    ...
    FROM production_tasks
    WHERE workshop_name IS NOT NULL AND workshop_name != ''
    GROUP BY workshop_name
    ORDER BY total_tasks DESC
    LIMIT 5
");
$workshopStats = $stmt->fetchAll();

// Топ-5 продукции по количеству заказов
$stmt = $db->query("
    SELECT 
...
<body>
    ...
    <script>
        ...
        // Chart.js - Orders Chart (Динамика заказов)
        const ordersCtx = document.getElementById('ordersChart').getContext('2d');
        const ordersGradient = ordersCtx.createLinearGradient(0, 0, 0, 300);
        ordersGradient.addColorStop(0, 'rgba(255, 107, 107, 0.6)');
        ordersGradient.addColorStop(1, 'rgba(255, 107, 107, 0.0)');
        ...
        new Chart(ordersCtx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [<?= implode(',', array_column($ordersByMonth, 'order_count')) ?>],
                    borderColor: '#FF6B6B',
                    ...
                    label: 'Сумма (тыс. руб)',
                    data: [<?= implode(',', array_map(fn($v) => round($v['total_amount'] / 1000), $ordersByMonth)) ?>],
                    borderColor: '#5ac8fa',
                    borderWidth: 2,
                    borderDash: [5, 5],
                    fill: false,
        ...
        // Chart.js - Status Chart (Распределение по статусам)
        ...
        new Chart(statusCtx, {
            ...
            data: {
                ...
                ?>],
                datasets: [{
                    data: [<?= implode(',', array_column($ordersByStatus, 'count')) ?>],
                    backgroundColor: [<?= implode(',', array_map(fn($s) => "'{$s['color']}'", $ordersByStatus)) ?>],
                    borderWidth: 0,
                    hoverOffset: 10
                }]
            },
