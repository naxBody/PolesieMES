<?php
/**
 * Модуль сотрудников - Редактирование сотрудника
 */
    ...
    if ($firstName && $lastName) {
        $stmt = $db->prepare("
            UPDATE staff SET 
                ...
                department = ?, email = ?, phone = ?, status = ?, updated_at = NOW()
            WHERE id = ?
        ");
        $stmt->execute([$firstName, $lastName, $middleName, $positionId, $department, $email, $phone, $status, $id]);
...
$pageTitle = 'Редактирование сотрудника | ' . APP_NAME;
?>
    ...
    <div class="main-content">
        <div class="page-header">
            <div class="page-title">
                <h1><i class="fas fa-edit"></i> Редактировать: <?= e($employee['last_name']) ?> <?= e($employee['first_name']) ?></h1>
            </div>
                ...
                <form method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="position_id" class="form-select">
                                ...
                                <option value="<?= $pos['id'] ?>" <?= $employee['position_id'] == $pos['id'] ? 'selected' : '' ?>><?= e($pos['name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select">
                            ...
                            <option value="vacation" <?= $employee['status'] == 'vacation' ? 'selected' : '' ?>>Отпуск</option>
                            <option value="sick" <?= $employee['status'] == 'sick' ? 'selected' : '' ?>>Больничный</option>
                            <option value="terminated" <?= $employee['status'] == 'terminated' ? 'selected' : '' ?>>Уволен</option>
                        </select>
                    </div>
